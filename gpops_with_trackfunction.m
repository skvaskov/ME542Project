...
s0=Track.arc_s(start); %initial arc length of centerline
sF=Track.arc_s(fin); %final arc length of centerline
T=(sF-s0)/10+t0;
...
bounds.phase(iphase).initialstate.lower = [xc0' 0 0];
bounds.phase(iphase).initialstate.upper = [xc0' 0 0];
 bounds.phase(iphase).state.lower = [-400 -300 -2*pi 0 -.5]; 
 bounds.phase(iphase).state.upper = [1600 1000 2*pi 150 .5]; 
...
%path constraints
bounds.phase(iphase).path.lower=
bounds.phase(iphase).path.upper=
%terminate trajectory in box of track width around centerline point
w=Trac.fun_width(sF);
bounds.eventgroup(iphase).lower=[xcF(1)-w/2 xcF(2)-w/2];
bounds.eventgroup(iphase).upper=[xcF(1)+w/2 xcF(2)+w/2];

%-------------------------------------------------------------------------%
%                          Initial Guess                                  %
%-------------------------------------------------------------------------%
%get initial semi-feasbile trajectory (from 542 project)
