...
bounds.phase(iphase).initialstate.lower = [xc0' 0];
bounds.phase(iphase).initialstate.upper = [xc0' 0];
bounds.phase(iphase).state.lower = [-400 -300 -pi 0]; 
bounds.phase(iphase).state.upper = [1600 1000 pi 150]; 
bounds.phase(iphase).finalstate.lower =  [-400 -300 -pi 0];
bounds.phase(iphase).finalstate.upper = [1600 1000 pi 150]; 
...
%path constraints
bounds.phase(iphase).path.lower=[-1e6 0];
bounds.phase(iphase).path.upper=[0 1e6];
...
guess.phase(iphase).state    = [ xc0' 0; xcF' 0];
...
gamma=input.phase.control(:,2);
dyn=[v.*cos(psi),v.*sin(psi),gamma,R/660];
phaseout.dynamics=dyn;
